...
# Enable CORS for all domains and origins
...
def save_data():
    global _leaderboard, _user_store

    # Only save data in the main worker process, not the reloader process
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
        print(f"Saving data... (PID: {os.getpid()})")

        with open("leaderboard.json", "w", encoding="utf-8") as f:
            json.dump(_leaderboard, f, ensure_ascii=False, indent=4)
            print("Saved leaderboard data", _leaderboard)

        with open("users.json", "w", encoding="utf-8") as f:
            json.dump(_user_store, f, ensure_ascii=False, indent=4)
            print("Saved user data", _user_store)
    else:
        print(f"Skipping save in reloader process (PID: {os.getpid()})")


def load_data():
    global _leaderboard, _user_store
    print(f"Loading data... (PID: {os.getpid()})")

    if Path("leaderboard.json").is_file():
        with open("leaderboard.json") as f:
            _leaderboard = json.load(f)
        print("Loaded leaderboard data", _leaderboard)

    if Path("users.json").is_file():
        with open("users.json") as f:
            _user_store = json.load(f)
        print("Loaded user data", _user_store)
